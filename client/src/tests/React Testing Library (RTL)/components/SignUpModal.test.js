...
import { loginUserWithGoogle, signupUserWithEmailPassword } from "../../../features/auth/authSlice";
...
describe('SignUpModal Component', () => {
    ...
    // Render Test
    it('renders the sign up modal correctly', () => {
        render(
    ...
    it('receives props correctly', () => {
        ...
        render(
            <Provider store={store}>
                <SignUpModal open={true} onClose={onClose} />
            </Provider>
        );
        expect(screen.getByText('Sign Up')).toBeInTheDocument();
    });
    ...
    it('handles email and password sign up correctly', () => {
        ...
        expect(store.getActions()).toContainEqual(signupUserWithEmailPassword({ email: 'test@example.com', password: 'password123' }));
